...
class _HomeScreenState extends State<HomeScreen> {
                  ...
                  if (i < latestManga.length) {
                    return Padding(
                      ...
                      child: GestureDetector(
                        onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => MangaDetailScreen(slug: latestManga[i].slug, preloadTitle: latestManga[i].title, preloadCover: latestManga[i].coverURL))),
                        child: Container(
                          padding: const EdgeInsets.all(12),
                          decoration: BoxDecoration(color: AppTheme.card, borderRadius: BorderRadius.circular(12)),
                          child: Row(
                            children: [
                              ClipRRect(borderRadius: BorderRadius.circular(12), child: CachedMangaImage(url: latestManga[i].highQualityCoverURL, width: 80, height: 110)),
                              const SizedBox(width: 12),
                              Expanded(
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text(latestManga[i].title, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: AppTheme.textPrimary), maxLines: 2),
                                    if (latestManga[i].latestChapterNumber != null) ...[
                                      const SizedBox(height: 4),
                                      Text('Chapter ${latestManga[i].latestChapterNumber}', style: const TextStyle(color: AppTheme.accent, fontSize: 12)),
                                    ],
                                    if (latestManga[i].lastUpdated != null) ...[
                                      const SizedBox(height: 2),
                                      Text(latestManga[i].lastUpdated!, style: const TextStyle(color: AppTheme.textTertiary, fontSize: 11)),
                                    ],
                                  ],
                                ),
                              ),
  ...
  Widget _sectionLabel(String title, IconData icon) => Row(children: [
        ...
        Text(title, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: AppTheme.textSecondary, letterSpacing: 1.5)),
      ]);
}
            ...
            child: Column(
              ...
              children: [
                ...
                Row(children: [
                  ...
                  Text('Ch.${progress.chapterNumber} · p.${progress.pageIndex + 1}', style: const TextStyle(fontSize: 10, color: AppTheme.accentBright)),
                ]),
              ],
            ),
          ),
    ...
    return SizedBox(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(manga.title, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: AppTheme.textPrimary), maxLines: 2),
        ],
      ),
    );
  }
